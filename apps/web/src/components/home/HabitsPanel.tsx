...
// the weekly habit list + DailyBean mood grid, or the task board. All navigation
// and mutations are delegated to HomeView via callbacks so this stays a pure view.
    ...
    <section ref={scrollRef} className={`flex-[1.3] min-h-0 bg-earth-bg p-6 md:p-8 pb-28 overflow-y-auto transition-opacity duration-300 ${isNavigating ? "opacity-50 pointer-events-none" : ""}`}>
      {activeTab === "habits" ? (
        ...
        <div className="space-y-6">
          ...
          <TaskBoard tasks={data.tasks || []} onRefresh={onRefresh} />
        </div>
      )}
    </section>
  );
